...
return true;
    ...
    const handleDelete = (id: number) => {
        if (confirm('Yakin ingin menghapus data ini?')) {
            router.delete(route(auth.user.role === 'admin' ? 'admin.pilah-sampah.show' : 'petugas.pilah-sampah.show', { pilah_sampah: id }));
        }
    };
...
params.set('custom_end', customEndDate);
        ...
        return route(auth.user.role === 'admin' ? 'admin.pilah-sampah.index' : 'petugas.pilah-sampah.index') + '/export' + (qs ? `?${qs}` : '');
    }, [search, filterJenis, filterPeriod, weekRange, selectedMonth, selectedYear, customStartDate, customEndDate]);
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button asChild className="bg-green-600 hover:bg-green-700">
                            <Link href={route(auth.user.role === 'admin' ? 'admin.pilah-sampah.create' : 'petugas.pilah-sampah.create')} className="flex items-center gap-2">
                                                    ...
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-2">
                                                            <Button variant="outline" size="sm" asChild className="border-green-200 text-green-700 hover:bg-green-50 hover:text-green-800">
                                                                <Link href={route(auth.user.role === 'admin' ? 'admin.pilah-sampah.edit' : 'petugas.pilah-sampah.edit', { pilah_sampah: item.id })} className="flex items-center gap-1">
