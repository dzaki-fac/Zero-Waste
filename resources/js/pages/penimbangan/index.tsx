...
return true;
    ...
    const handleDelete = (id: number) => {
        if (confirm('Yakin ingin menghapus data ini?')) {
            router.delete(route(auth.user.role === 'admin' ? 'admin.penimbangan.show' : 'petugas.penimbangan.show', { penimbangan: id }));
        }
    };
...
params.set('custom_end', customEndDate);
        ...
        return route(auth.user.role === 'admin' ? 'admin.penimbangan.index' : 'petugas.penimbangan.index') + '/export' + (qs ? `?${qs}` : '');
    }, [search, filterArea, filterJenis, filterPeriod, weekRange, selectedMonth, selectedYear, customStartDate, customEndDate]);

    return (
        <>
            <Head title="Tabel Penimbangan" />

            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <Heading
                        title="Tabel Penimbangan"
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button asChild className="bg-green-600 hover:bg-green-700">
                            <Link href={route(auth.user.role === 'admin' ? 'admin.penimbangan.create' : 'petugas.penimbangan.create')} className="flex items-center gap-2">
                                                ...
                                                <TableCell className="text-right">
                                                    <div className="flex justify-end gap-2">
                                                        <Button variant="outline" size="sm" asChild className="border-green-200 text-green-700 hover:bg-green-50 hover:text-green-800">
                                                            <Link href={route(auth.user.role === 'admin' ? 'admin.penimbangan.edit' : 'petugas.penimbangan.edit', { penimbangan: item.id })} className="flex items-center gap-1">
