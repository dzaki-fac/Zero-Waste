...
return true;
    ...
    const handleDelete = (id: number) => {
        if (confirm('Yakin ingin menghapus data ini?')) {
            router.delete(route(auth.user.role === 'admin' ? 'admin.distribusi.show' : 'petugas.distribusi.show', { distribusi: id }));
        }
    };
...
params.set('custom_end', customEndDate);
        ...
        return route(auth.user.role === 'admin' ? 'admin.distribusi.index' : 'petugas.distribusi.index') + '/export' + (qs ? `?${qs}` : '');
    }, [search, filterJenis, filterTujuan, filterPeriod, weekRange, selectedMonth, selectedYear, customStartDate, customEndDate]);
...
return;
        ...
        router.patch(
            route(auth.user.role === 'admin' ? 'admin.distribusi.review' : 'petugas.distribusi.review', { distribusi: id }),
            { status, note: null },
            {
                preserveScroll: true,
...
return;
        ...
        router.patch(
            route(auth.user.role === 'admin' ? 'admin.distribusi.review' : 'petugas.distribusi.review', { distribusi: revisionDistribusiId }),
    ...
    }, [auth.user.role, revisionDistribusiId]);
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button asChild className="bg-green-600 hover:bg-green-700">
                            <Link href={route(auth.user.role === 'admin' ? 'admin.distribusi.create' : 'petugas.distribusi.create')} className="flex items-center gap-2">
                                                        ...
                                                        <div className="flex justify-end gap-2">
                                                            {canEdit(item) && (
                                                                <Button variant="outline" size="sm" asChild className="border-green-200 text-green-700 hover:bg-green-50 hover:text-green-800">
                                                                    <Link href={route(auth.user.role === 'admin' ? 'admin.distribusi.edit' : 'petugas.distribusi.edit', { distribusi: item.id })} className="flex items-center gap-1">
